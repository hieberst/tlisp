...
Unter Windows wird TLISP mittels der Batch-Datei \path{tlisp.cmd}\index{tlisp.cmd} gestartet.

\subsubsection{Android}
Am Einfachsten ist die Installation im Heimatverzeichnis von Gforth.
\begin{enumerate}
\item Installation von Gforth aus dem \href{https://play.google.com/store/apps/details?id=gnu.gforth}{Google Play Store} und Ausführen von Gforth
      zum Entpacken des Dateien.
\item Ermitteln des Heimatverzeichnisses \verb|HOME| von Gforth
        \begin{verbatim}
        sh pwd
        \end{verbatim}
\item Kopieren der Dateien \path{tlisp.fs} und \path{*.lsp} in das Verzeichnis \path{~/tlisp}.
\item Erzeugen der Datei \path{~/.config/gforthrc} mit folgendem Inhalt:
        \begin{verbatim}
        0.5e gl-fscale    \ SM-T580 only

        : tl    \ run tlisp on my android handy
            warnings off    \ 0. vs. #0.
            s" tlisp" set-dir drop
            s" tlisp.fs" required
            s" driver" evaluate
        ;
        \end{verbatim}
      Der Poettering-Stil wird ab Gforth Version \href{https://github.com/forthy42/gforth/releases/tag/0.7.9_20180905}{0.7.9\_20180905}
      verwendet, vorher hieß diese Datei \path{~/.gforthrc}.
\end{enumerate}
Eine gute Tastatur ist das \href{https://play.google.com/store/apps/details?id=org.pocketworkstation.pckeyboard}{Hacker's Keyboard}.
...
\item \path{tlisp.btm} \hspace{4mm} Batch-Datei für 4DOS, 4NT und TCC
\item \path{tlisp.ahk} \hspace{4mm} \href{https://www.autohotkey.com/}{AutoHotkey}-Skript für Windows
\item \path{tlisp.sub} \hspace{4mm} Submit-Datei für CP/M
\end{description}
